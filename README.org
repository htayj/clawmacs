...
| =clawmacs_reload= | agent-allowed     | Safely preflight and reload Clawmacs source in place |
...
=clawmacs_reload= gives agents and users a safer hot-reload path for updated
Clawmacs source. It first loads the current checkout in an isolated SBCL worker;
