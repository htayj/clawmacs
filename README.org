...
Clawmacs is a Lisp-native, Emacs-inspired LLM chat interface written entirely in Common Lisp. It gives you and your AI agent a shared, programmable environment while keeping provider work off the CLIM frame process. The agent can read, write, and edit files directly through Lisp data mode, and it can use isolated =lisp_eval= workers to test Common Lisp without freezing or terminating the live UI.
...
Clawmacs is a full-trust application. Agent tools, package code, init files,
shell commands, and live Lisp all run with the operating-system authority of
the Clawmacs process. There is deliberately no in-application permission prompt
...
without mutating or crashing the live Clawmacs image. The worker inherits the
user's filesystem and network authority; this separation is for application
stability, not containment.

The =safe-reload-clawmacs-command= gives users a deliberate development
hot-reload path for updated Clawmacs source. It is not exposed as a provider
...
only if that preflight succeeds does it reload =:clawmacs= in the live image,
...
resources unless other Clawmacs APIs explicitly execute or use them. Skill
...
Clawmacs runs as a McCLIM application frame with a presentation-rich transcript
...
=CLAWMACS_TRUETYPE_FONT_PATH= to a compact font directory if you need an
override. Set =CLAWMACS_SBCL_DYNAMIC_SPACE_SIZE= to adjust SBCL's heap size in
...
key is configured. Without a successful live fetch, Clawmacs falls back to a
...
OpenRouter routes; Clawmacs does not provide direct adapters for them.
...
(setf clawmacs:*default-model* "gpt-5.6-sol")
(setf clawmacs:*openai-codex-model* "gpt-5.6-sol")
