...
The default built-in agent tool surface is provided by the =lispi= package and
follows Pi-style coding tools, with =lisp_eval= replacing shell access:
...
By default, agents use the file tools for workspace search and text changes and
=lisp_eval= for Lisp-native inspection, documentation lookup, computation, and
runtime updates. Tool inputs and tool results are represented as Lisp data, for
example =(edit :path "src/foo.lisp" :old-text "old" :new-text "new")=.
The =write= and =edit= tools reject content that leaves Lisp parentheses
unbalanced.
Custom tools may still be registered explicitly.
