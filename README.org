...
Clawmacs is a Lisp-native, Emacs-inspired LLM chat interface written entirely in Common Lisp. It gives you and your AI agent a shared, programmable environment with full access to the running Lisp image. By default the agent uses explicit project and documentation tools for routine work, with =lisp_eval= available for structured editing, function calls, and runtime inspection inside the live Common Lisp process.
...
Clawmacs now has a few stable concepts that describe how the interface and
agent workflow fit together:

- *Image-native agent loop* --- the agent works inside the live Common Lisp
  image through project/docs tools plus =lisp_eval=, so inspect/edit/verify
  happens in one runtime.
- *Recency-biased fuzzy selectors* --- minibuffer and overlay selection UIs
  share the same paradigm: fuzzy matching plus recency ordering, with the
  current item de-prioritized when reselection is unlikely.
- *Project resource workflow* --- durable edits happen through project
  resources and transactional change sets instead of ad hoc filesystem writes.
- *Structural editing for Lisp* --- =sexed= edits forms by structure and
  validates balanced results, giving agent-safe transformations for Lisp code.
- *Backend-neutral command layer* --- the same command and rendering concepts
  apply across Croatoan (terminal) and McCLIM (graphical) backends.
...
| =project_list_files= | agent-allowed | List project resource paths |
| =project_read_file=  | agent-allowed | Read a full project resource |
| =project_read_lines= | agent-allowed | Read focused project resource lines |
| =project_search=     | agent-allowed | Search project resources |
| =project_write_file= | agent-allowed | Save a full project resource |
| =doc_lookup=         | agent-allowed | Look up Clawmacs, Common Lisp, and system docs |

By default, agents use the project and documentation tools for routine
read/write/search/docs work, and use =lisp_eval= as the escape hatch for
structured edits, custom function calls, runtime inspection, and operations not
covered by narrower tools. Custom tools may still be registered explicitly.

The =lisp_eval= tool is particularly powerful: the agent can introspect clawmacs itself, query buffer state, evaluate expressions, and modify the running system. Standard Common Lisp reference lookup is available through a vendored offline snapshot of =cl-community-spec=, and imported library discovery is handled locally through ASDF metadata, package exports, docstrings, and source/doc search helpers.

=lisp_eval= captures printed stdout/stderr as well as multiple return values.
Recent calls are available with =(eval-history-to-string)=, and the latest
success/error are exposed through =*last-eval-result*= and
=*last-eval-condition*=.
