...
layout has a presentation-rich transcript pane, a Drei-backed input pane, a
who-line, a modeline, and a fixed minibuffer stream for command input. The
interface uses CLIM panes, commands, presentations, queued events, pulse polling,
and redisplay for the interactive UI while Clawmacs keymaps continue to own
editor keystrokes.
