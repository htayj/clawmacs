...
The McCLIM interface follows a Lisp-machine style pane layout: transcript,
compose header, editable input pane, modeline, who-line, and native pointer
documentation. Object interaction is presentation-driven, so messages, package
entries, TODOs, selectors, manuals, and other semantic objects can be used with
the mouse as well as keys.
...
Clawmacs runs as a McCLIM application with an ESA-backed application frame. The
layout has a presentation-rich transcript pane, a dedicated compose pane, a
Drei-backed input pane, a who-line, a modeline, a native pointer
documentation pane, and a hidden ESA minibuffer service pane. The interface
uses CLIM panes, commands, presentations, pointer documentation, queued
events, pulse polling, and redisplay for the interactive UI while Clawmacs
keymaps continue to own editor keystrokes.
...
src/
  ...
  mcclim-app.lisp      McCLIM application frame, panes, events, redisplay
  debug.lisp           McCLIM debugger, Clouseau, and listener integration
  main.lisp            Commands, key dispatch, startup, prompt entrypoints
  docs.lisp            Extended documentation entries
#+end_example

The core architecture follows Emacs patterns: buffers hold messages (not file lines), keymaps dispatch to commands, faces control rendering, and McCLIM owns the interactive display.

The rendering layer is split: =render-core.lisp= contains pure geometry and formatting functions (line wrapping, modeline formatting, selector line layout), while =mcclim-app.lisp= translates that state into CLIM drawing, presentations, and redisplay.
