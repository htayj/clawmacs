...
Clawmacs is a Lisp-native, Emacs-inspired LLM chat interface written entirely in Common Lisp. It gives you and your AI agent a shared, programmable environment with full access to the running Lisp image. By default the agent acts through =lisp_eval=: it can inspect the editor, search symbols, read docs, call functions, and update runtime state directly inside the live Common Lisp process.
...
- *Image-native agent loop* --- the agent works inside the live Common Lisp
  image via =lisp_eval=, so inspect/edit/verify happens in one runtime.
