...
#+begin_src lisp
(sexed-file-outline-to-string "src/main.lisp" :head "defun" :limit 20)
(sexed-file-form-text "src/main.lisp" '(:head "defun" :name "ensure-scratch-buffer"))
...
=sexed-replace-form= return edited strings; file, message, and scratch adapters
validate the full result before mutating files or editable clawmacs messages.
