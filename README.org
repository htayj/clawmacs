...
Clawmacs is a Lisp-native, Emacs-inspired LLM chat interface written entirely in Common Lisp. It gives you and your AI agent a shared, programmable environment with access to project resources and the running Lisp image. By default the agent has only =read=, =write=, and =eval= tools; =eval= is reserved for tests/checks or live Clawmacs state when the narrower tools are insufficient.
...
- *Image-native agent loop* --- the agent uses narrow =read= and =write=
  tools for normal work, with =eval= available only when live Lisp state or
  checks require it.
...
|--------------+------------------------+--------------------------------------|
| =read=  | agent-allowed | List/read/search project resources and look up local docs |
| =write= | agent-allowed | Save a full project resource |
| =eval=  | agent-allowed | Evaluate Common Lisp only for checks or live Clawmacs state |

Agents should use =read= for inspection and documentation, =write= for durable
project edits, and =eval= only when running tests/checks or interacting with
live Clawmacs state is necessary. Additional capability should be added as
dedicated tools only after prompt testing shows a real need.

The =eval= tool is powerful: it can evaluate expressions and modify the running
system. It should not be used for ordinary file reads/writes/searches.

=eval= captures printed stdout/stderr as well as multiple return values.
...
Skills do not add new agent tools or widen permissions. Skill instructions are
injected into the conversation when mentioned or selected, and scripts/assets
are local resources unless other Clawmacs APIs explicitly execute or use them.
...
#+begin_src sh
./prompt.sh "Use read to summarize README.org."
...
(clawmacs:register-agent-definition
 ...
 :model "gpt-5.4"
 :think-level "high"
 :core-prompt "Use read and write for concrete work. Use eval only for checks."
