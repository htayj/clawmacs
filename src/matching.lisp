...
(defun matching-core-available-p ()
  ...
  (not (null (and (find-package '#:rplaca/matching-core)
                  (fboundp 'rplaca/matching-core:fuzzy-match-p)))))
...
independently match the candidate via subsequence search."
  (rplaca/matching-core:split-query-tokens query))
...
Both TOKEN and CANDIDATE should already be downcased."
  (rplaca/matching-core:fuzzy-token-match-p token candidate))
...
search.  An empty query (or all-spaces query) matches everything."
  (rplaca/matching-core:fuzzy-match-p query candidate))
...
be downcased.  Returns NIL if TOKEN does not match CANDIDATE."
  (rplaca/matching-core:fuzzy-token-positions token candidate))
...
from every token.  Returns NIL if any token fails to match or query is empty."
  (rplaca/matching-core:fuzzy-match-positions query candidate))
...
Returns NIL when TOKEN does not match.  Higher scores mean better matches."
  (let ((score (rplaca/matching-core:fuzzy-token-score-or-negative-one
                token candidate)))
    (unless (minusp score)
      score)))
...
if any token fails to match."
  (let ((score (rplaca/matching-core:fuzzy-score-or-negative-one
                query candidate)))
    (unless (minusp score)
      score)))
