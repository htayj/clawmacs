...
(defun make-legacy-path-test-root ()
  (let ((root (merge-pathnames
               (format nil "rplaca-legacy-path-~36R-~36R/"
                       (get-universal-time)
...
(test canonical-path-wins-without-merging
  (with-legacy-path-test-root (root)
    (let* ((canonical (merge-pathnames #P"rplaca/config.json" root))
      ...
      (let ((rplaca::*legacy-path-warning-function*
              (lambda (control &rest arguments)
                (push (apply #'format nil control arguments) warnings))))
        (rplaca::reset-legacy-path-warnings)
        (let ((selection
                (rplaca::select-migration-path
                 canonical legacy :label "test configuration")))
          (is (equal canonical
                     (rplaca::legacy-path-selection-read-path selection)))
          (is (equal canonical
                     (rplaca::legacy-path-selection-write-path selection)))
          (is (eq :conflict
                  (rplaca::legacy-path-selection-source selection)))
...
(test absent-path-selects-canonical-for-read-and-write
  (with-legacy-path-test-root (root)
    (let* ((canonical (merge-pathnames #P"rplaca/config.json" root))
           (legacy (merge-pathnames #P"clawmacs/config.json" root))
           (selection
             (rplaca::select-migration-path canonical legacy)))
      (is (equal canonical
                 (rplaca::legacy-path-selection-read-path selection)))
      (is (equal canonical
                 (rplaca::legacy-path-selection-write-path selection)))
      (is (eq :absent (rplaca::legacy-path-selection-source selection))))))
    ...
    (let* ((canonical (merge-pathnames #P"rplaca/config.json" root))
      ...
      (let ((rplaca::*legacy-path-warning-function*
        ...
        (dotimes (index 3)
          ...
          (rplaca::migration-read-path canonical legacy :label "test"))
        (is (= 1 (length warnings)))))))
...
(test canonical-created-during-probe-wins
  (with-legacy-path-test-root (root)
    (let* ((canonical (merge-pathnames #P"rplaca/config.json" root))
      ...
      (write-legacy-path-test-file legacy "legacy")
      (let ((rplaca::*migration-path-probe-function*
              (lambda (path)
...
(test migration-read-roots-never-merges
  (with-legacy-path-test-root (root)
    (let ((canonical (merge-pathnames #P"rplaca/" root))
      ...
      (let ((roots (rplaca::migration-read-roots canonical legacy)))
        (is (= 1 (length roots)))
        (is (equal canonical (first roots))))
      (is (equal canonical
                 (rplaca::migration-write-path canonical))))))
  ...
  (with-legacy-path-test-root (root)
    (let* ((canonical (merge-pathnames #P"rplaca/config.json" root))
      ...
      (let ((rplaca::*legacy-path-warning-function*
        ...
        (rplaca::reset-legacy-path-warnings)
        (is (eq :legacy
                (rplaca::legacy-path-selection-source
                 (rplaca::select-migration-path canonical legacy))))
        (is (eq :absent
                (rplaca::legacy-path-selection-source nested-selection)))))))
