# Clawmacs Feature-Gap Analysis

- **Compared products:** Codex CLI, Pi, and OpenCode
- **Research date:** 2026-07-14
- **Clawmacs snapshot:** working tree on `esa-conversion`, based on
...
OpenCode with the behavior implemented in the current Clawmacs working tree.
...
- **Present** means Clawmacs has an end-to-end user workflow, not merely a
...
- **Fit** measures compatibility with Clawmacs as a Lisp-native,
...
The comparison evidence below is retained even where Clawmacs deliberately
...
|---|---|
| In-application sandbox and permission prompts | **Rejected.** Clawmacs follows a Pi-like full-trust posture. Remove the incomplete permission/ask-for-permission system rather than presenting partial policy as security. Users who need containment should launch Clawmacs in an external sandbox or container that they control. |
...
| Shared comparator gap 4 | **Deferred.** Do not add an application updater before Clawmacs has releases, a release format, channels, and a distribution contract. Current users build a private pre-alpha from source. |
...
image, including code that can modify Clawmacs itself, without asking the
harness to enforce a boundary it can also rewrite.

## Bottom Line

Clawmacs is not a thin chat client waiting to copy the competitors. It already
...
1. **OS-enforced command and network isolation exists in Codex, while the
   removed Clawmacs guard settings were only authorization metadata.** This is
...
2. **Turn-coupled undo and redo.** Clawmacs has change sets and recovery
...
5. **Configurable custom/local provider reach.** OpenRouter provides broad
   model access, but Clawmacs itself has fixed production transports. The
...
6. **Comparator-specific plan and policy workflows.** Their existence is
   recorded, but Clawmacs chooses a general interaction-mode facility and no
...
| Capability | Clawmacs evidence | Comparable products |
|---|---|---|
...
| Basic MCP client with one-shot stdio subprocess requests and JSON-RPC-over-HTTP POST, including Clawmacs tool/resource discovery | `src/mcp-bridge-core.lisp`, `packages/channels/default/mcp-bridge/` | Codex and OpenCode are comparable MCP tool clients; explicit resource-discovery parity is not established here. Clawmacs does not yet provide persistent stdio, Streamable HTTP/SSE sessions, notifications, or OAuth; Pi intentionally leaves MCP to extensions |
...
Pi, and OpenCode but missing in Clawmacs.” The first three are partial. The
...
supports them. Clawmacs' Artifactum accepts image MIME types, copies the files
...
The right Clawmacs shape is to add an attachment domain object and provider
...
Failing before submission is a deliberate stronger Clawmacs contract, not Pi
...
Clawmacs can insert an entire project file, attach a local file, and insert
...
capabilities. Clawmacs supports OpenAI Codex, Z.AI, and OpenRouter directly.
...
- **State:** Missing for Clawmacs itself; installed packages can be updated
...
- **Roadmap:** Deferred until Clawmacs has actual releases

Codex has a stable `codex update` command, Pi provides `pi update --self`, and
OpenCode provides `opencode upgrade`. Clawmacs can update installed packages and
...
Clawmacs is currently a private, pre-alpha, source-built application. There is
...
the old Clawmacs permission surface. They are **not backlog items**. Clawmacs
...
Clawmacs process under an OS sandbox, container, VM, restricted account, or
other boundary of their choice. This is compatible with tools and packages
being able to modify Clawmacs itself while keeping the enforcement mechanism
...
Clawmacs is normally launched inside a Guix container, which can provide useful
...
justify building a second sandbox inside Clawmacs. The labels, policy
decisions, and approval workflow have now been removed. Current documentation
states plainly that commands inherit the authority of the Clawmacs process and
...
The snapshot implementation lacks argument matching, but Clawmacs will not add
...
- **Gain:** An enabled project package or hook cannot silently execute changed
  code with the Clawmacs process's full authority
- **Effort:** M
...
with full process authority. Clawmacs records package source and activation
...
Git changes; `/redo` restores them. Clawmacs has reversible change sets and a
...
Clawmacs change set. Run a reviewer agent with an explicitly read-only tool set.
...
investigation proposal, not an accepted API contract. Clawmacs is heavily in
...
Clawmacs will not ship Codex- or Claude-style plan mode as the core feature.
...
containment. Plugins and tools retain the full authority of the Clawmacs
...
| Gap | Seen in | Current Clawmacs state | Fit and gain | Effort / caution |
|---|---|---|---|---|
...
| **G11. Session archive/delete and versioned exchange** | Codex lifecycle; Pi JSONL import/export; OpenCode delete and JSON/share-URL import/export | Partial: load, resume, fork, rename, HTML export, and local share exist | **High.** Session hygiene, migration, backup, and reproducible bug reports | **S-M.** Archive by metadata/index first; deletion must require explicit confirmation and protect open sessions. A versioned portable contract is a new Clawmacs requirement; neither a product's JSON export nor Clawmacs' internal sidecars should be assumed stable without one |
| **G12. Application release/update lifecycle** | Codex `update`; Pi `update --self`; OpenCode `upgrade` | Missing for Clawmacs itself; installed-package updates and safe reload exist | **Deferred until releases exist.** It has no useful input contract while users build the private pre-alpha from source | **M plus release work.** Reconsider only after release format, channel, provenance, restart, migration, and rollback decisions exist |
...
| **G16. Session exchange and revocable sharing** | Pi JSONL exchange and private/unlisted-Gist sharing; OpenCode JSON/share-URL import/export plus public share/unshare. Restricted/self-hosted OpenCode sharing is enterprise-only | Partial: HTML export and pluggable local/hook handlers exist | **Medium.** Easier collaboration and issue reproduction | **M.** Self-hosting, redaction preview, expiry, and authenticated privacy are proposed Clawmacs strengthening. Public/access-by-link should remain off by default; validate imports before opening them |
| **G17. MCP server exposure** | Codex CLI | Missing: Clawmacs is an MCP client only | **Medium.** Other editors and agents could delegate a Clawmacs thread/review/session operation | **M.** If revived, adapt the interop core and expose a deliberately small surface with explicit cancellation and full-trust process semantics; do not promise an in-app sandbox |
...
| OpenCode TUI controls, terminal themes/widgets, or external-editor escape | Pi and OpenCode | Low | These solve terminal-client constraints that Clawmacs does not have. Recreate useful semantics with CLIM commands, presentations, completion, Drei/editor behavior, and faces—not their rendering layers. |
| Cloud-task bridge | Codex CLI (`codex apply` stable; `codex cloud` experimental) | Low without a Clawmacs cloud service | This is a real CLI feature, but it depends on a remote task product rather than a local editor primitive. Treat it as a deliberate scope exclusion until Clawmacs has a remote-execution design. |
...
workflow informs the need for a visible mode concept, but Clawmacs generalizes
...
are already represented in Clawmacs. Memory, doctor, and MCP-server mode remain
...
Codex's OS sandbox and policy rules remain valid comparator facts, but Clawmacs
...
well represented in Clawmacs. The remaining useful gaps are native images,
...
TypeScript extension model is not a security boundary. Clawmacs adopts the same
...
Clawmacs will expose a general interaction-mode contract and leave any plan
...
confirm the Clawmacs capability against the working tree again, because several
