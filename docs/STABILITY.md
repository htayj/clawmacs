...
RPLACA is a full-trust application. The former in-process permission,
...
are classified as harness faults, not RPLACA or McCLIM crashes.
...
| P1 | Legacy or externally generated Artifactum indexes without `updated_at` reached a parallel `LET` initializer that referenced `created-at` before it was bound, signalling `UNBOUND-VARIABLE`. | RPLACA | Normalize timestamps with sequential binding and fall back to the supplied or generated creation timestamp when `updated_at` is absent. | Four regressions cover missing creation/update combinations and durable legacy-index reading; the focused Artifactum suite completes 51/51 checks. |
...
| P1 | Appearance persistence and restart-only activation could look correct in one live frame while leaking staged state, port font state, or an old Xvfb artifact into a later run. | Harness / RPLACA boundary | Emit data-only frame-owned active/staged/persisted themes, catalog/profile/font/bundle generations, activation classification, and resolved portable surfaces. Run the appearance proof in two separate Guix containers with fresh private Xvfb servers; preserve only artifact-private HOME/cache between them. | The `appearance` GUI suite requires cold `classic`, `dark` staging with no active mutation, `restart-required` Apply rollback, explicit Save, then a separate startup with a coherent dark bundle. It also proves the public CLX font inventory refresh and both compatibility editor bindings. |
...
- the mocked launcher cache regression plus a real non-preflight warmup and
  parallel GUI launch (preflight intentionally does not load `:rplaca`);
- a fresh ASDF load of `:rplaca/tests`;
- the full `rplaca-suite` repeatedly, with zero failures and no worker from
...
- real Xvfb `smoke`, `mx`, `features`, `keybinds`, `organa`, `quaestor`,
  `reload`, `appearance`, and `stability` suites;
...
12. Package, configuration, project, manifest, and change-set locks
    ...
    but two RPLACA processes can still produce last-writer-wins updates.
...
14. McCLIM Drei has an independently reproducible renderer defect: an ordinary
    ...
    `866003f` removes RPLACA's repaint-time compose relayout and declares fixed
...
- A fresh `:rplaca/tests` image ran `rplaca-suite` twice without restarting
...
- Explicit provenance resolved `clim-core`, `clim`, `mcclim`, `mcclim-clx`,
  `esa-mcclim`, and `drei-mcclim` to the pinned Guix store tree. The RPLACA-free
...
- One fresh Lisp image ran the complete `rplaca-suite` twice. Each pass
...
- Shell syntax, Python compilation, and `git diff --check` passed. The removed
  ...
  that `rplaca_reload` is absent from the provider tool table.
...
- one fresh Guix Lisp image ran the full `rplaca-suite` twice. Each pass
...
- the complete seeded real-GUI matrix passed in controlled concurrent pairs:
  ...
  `RPLACA_GUI_E2E_COLD_CACHE=1` smoke also passed from an empty private cache.
...
- all six tested CLIM/ESA/Drei systems resolved to the pinned Guix McCLIM tree.
  The RPLACA-free ESA control used a private Xvfb inside Guix and again
...
not the operational fork threshold because RPLACA's stable command-table
...
Every other new crash, race, and performance cost above was owned by RPLACA,
...
switch-buffer repair. A RPLACA-free standard `:drei` pane with McCLIM's
